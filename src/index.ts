...
/**
 * Custom handler function for processing specific node types during slicing
 */
export interface NodeHandler {
  (node: Node, context: SliceContext): Node | Node[] | null;
}

/**
 * Context information passed to custom node handlers
 */
 ...
 * @param end - Ending character position (exclusive)
...
export function sliceMarkdown(
  ...
  if (end <= start) throw new Error("End position must be greater than start");
  if (!Number.isInteger(start) || !Number.isInteger(end)) {
    throw new Error("Positions must be integers");
  }
  ...
  function slice(node: Node): Node | Node[] | null {
    ...
    // Use custom handler if one is provided for this node type
    const customHandler = resolvedOptions.handlers[node.type];
    if (customHandler) {
      const result = customHandler(node, {
        start,
        end,
        nodeStart,
        nodeEnd,
  ...
  function sliceFormattingNode(
    ...
    const overlapsStart = nodeStart < end && nodeEnd > start;
    if (!overlapsStart) {
      return null;
    }

    // Determine if this is a partial slice (node extends beyond slice boundaries)
    const isPartial = nodeStart < start || nodeEnd > end;
  ...
  function sliceInlineCode(
    ...
    return value.length === 0 ? null : Object.assign({}, node, { value });
  }
  ...
  function sliceImageNode(
    ...
    const behavior =
      resolvedOptions.behavior[node.type as "image" | "imageReference"];
    const isPartial = nodeStart < start || nodeEnd > end;
   ...
   */
  function sliceAtomicNode(
    node: Node,
    nodeStart: number,
    nodeEnd: number,
  ): Node | null {
    // Include atomic nodes if they overlap with the slice range
    return nodeEnd > start && nodeStart < end ? node : null;
  }
...
function getNodeLength(node: Node): number {
  ...
  if (type === "text" || type === "inlineCode") {
    // Text nodes and inline code contribute their string length
    length = (node as any).value.length;
  } else if (type === "code") {
    // Code blocks contribute their content length
    length = (node as any).value.length;
